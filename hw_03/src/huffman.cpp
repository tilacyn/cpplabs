...
void HuffTree::sort(){
    std::sort(source.begin(), source.end(), compare1);
}
...
long long HuffmanArchiver::get_symcount(){
    return symcount;
}
...
long long HuffmanArchiver::get_archive_size(){
    return archive_size;
}
...
void HuffmanArchiver::decode(std::istream& ifs, std::ostream& ofs){
    ...
    while(ifs.peek() != std::istream::traits_type::eof() || ccount < symcount){
        ...
        if(j < 0) j = 7, ifs.read((char*)&rbyte, 1), archive_size++;
        if(rbyte & (1 << j)){
            s += "1";
        } else s += "0";
        j--;
